...
//         let rpc_user: String = env::var("Davidpere").expect("BITCOIN_RPC_USER must be set");
...
fn time_to_mine(block_height: u64) -> Duration {
    ...
    rpc_client.get_block_hash(234).expect("TODO: panic message");
    todo!()
}
...
fn main() {
    ...
    // println!("{:?}", res);
    // const TIMEOUT_UTXO_SET_SCANS: time::Duration = time::Duration::from_secs(60 * 4); // 4 minutes
    // dotenv::dotenv().ok();
    // let rpc_url: String = env::var("BITCOIN_RPC_URL").expect("BITCOIN_RPC_URL must be set");
    ...
    let acc_balance = rpc_client.get_balances();
    println!("{:#?}",acc_balance);
    println!("{:?}", res);
}
